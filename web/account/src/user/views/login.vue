...
<div class="login-page">
  <div class="login">
        ...
        <div class="login__form">
          ...
          <button type="button" class="login__submit" @click="doLogin" :class="{processing: isProcessing, success: isSuccess}">登 录</button>
          <p class="login__signup">还没有账号? &nbsp;
...
<script>
  ...
  // import webSocket from 'common/utils/websocket.js'
  ...
  export default {
    ...
    methods: {
      ...
      login: function () {
        ...
        let that = this
        httpPost(Constants.SYSTEM.USR, 'doLogin', param).then((data) => {
          console.log('success')
          that.isSuccess = true
          setTimeout(function () {
            let path = that.$route.params.redirect
            console.log(path)
            that._logon({user: {name: this.name}})
            if (path) {
